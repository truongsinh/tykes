...
# If behave such that don't return untranslated entities at all
if settings.FALL_BACK_TO_DEFAULT == 2:
	class MultilingualManager(models.Manager):
		def all(self):
			return self.filter(translations__language_code=get_language())

		def count(self):
			return self.filter(translations__language_code=get_language()).count()

		# TODO: Tons of other Manager methods need to be extended

class MultilingualModel(models.Model):
	if settings.FALL_BACK_TO_DEFAULT == 2: objects = MultilingualManager()
		...
		# for field in self._meta.multilingual:
			...
			if code:
				try:
					...
					if settings.FALL_BACK_TO_DEFAULT == 1 and\
	...
	def unicode_wrapper(self, property, default=ugettext('Untranslated')):
		"""
